view: CDG_TAX_PTE_PUSH_Monthly {
  ...
  derived_table: {
    ...
    sql:        select
            ...
            --CAST(Created_DateTB01 AS DATE) as SLA_Date ,
            ...
            --          SUM( casefunction )/SUM(orderidcount)  as Avg_Resp_Time,
              --        SUM( Ttl_Request_Messages) AS Total_Request_Messages,
                     ( SUM(LESSTHAN_OR_EQUAL_10_SECS)/SUM( ORDER_COUNT_Div))*100 AS LESSTHAN_OR_EQUAL_10_SECS,

                      (SUM(GREATERTHAN_15_SECS)/SUM( ORDER_COUNT_Div))*100 AS GREATERTHAN_15_SECS,

                      (SUM(BETWEEN_10_15_SECS)/SUM( ORDER_COUNT_Div))*100 AS BETWEEN_10_15_SECS,
            ...
            --Count({<SLA_Month_Date={">=$(=vSLAStartDate)<=$(=vToday) "}, Created_DateTB03={'*'} >}SLA_orderId)
             ...
             ( CASE WHEN DATE_DIFF(cast(Created_DateTB02 as timestamp),cast(SLA_Month_Date as timestamp),millisecond) <= 10000 then 1 else 0 end)  AS LESSTHAN_OR_EQUAL_10_SECS,

             ( CASE WHEN
              ( DATE_DIFF(cast(Created_DateTB02 as timestamp),cast(SLA_Month_Date as timestamp),millisecond) > 10000 and
                DATE_DIFF(cast(Created_DateTB02 as timestamp),cast(SLA_Month_Date as timestamp),millisecond) <= 15000 )

             then 1 else 0 end)  AS BETWEEN_10_15_SECS,
    ...
    measure: LESSTHAN_OR_EQUAL_10_SECS{
      type: number
      sql: SUM(${TABLE}.LESSTHAN_OR_EQUAL_10_SECS) ;;
    }
    ...
    measure: BETWEEN_10_15_SECS {
      type: number
      sql: SUM(${TABLE}.BETWEEN_10_15_SECS) ;;
    }
    ...
    dimension: Tax_Date_Dimension {
      label: "Tax Date"
      sql: ${TABLE}.Tax_Date_Dimension ;;
    ...
    dimension_group: Tax_Days_Date {
      ...
      # sql: TIMESTAMP(datetime(cast(${TABLE}.Tax_Days_Date as timestamp),"America/Los_Angeles")) ;;
    }
